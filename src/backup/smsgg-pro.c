...
Copyright (c) 2004 - 2005, 2015 - 2017, 2019 - 2021 dbjh
...
int
smsgg_read_rom (const char *filename, unsigned short parport, unsigned int size)
{
  FILE *file;
...
smsgg_write_rom (const char *filename, unsigned short parport)
{
  ...
  int game_no;
  unsigned int size, address = 0, multi_game;
  size_t bytesread, bytessent = 0, bytesleft = 0;
  time_t starttime;
...
smsgg_read_sram (const char *filename, unsigned short parport, int start_bank)
{
  FILE *file;
  unsigned char buffer[0x100];
  unsigned int blocksleft, address, size, bytesreceived = 0;
  time_t starttime;
  void (*read_block) (unsigned int, unsigned char *) = ttt_read_ram_b; // ttt_read_ram_w

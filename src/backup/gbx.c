...
static int cart_type = 0;                       // should be set to value of ROM[0x147]
...
gbx_write_rom (const char *filename, unsigned short parport)
{
  ...
  if ((ucon64.fsize < 0x8000) || (ucon64.fsize & 0x7fff) ||
      (ucon64.fsize > 64 * MBIT))
    {
      fputs ("ERROR: File size error\n", stderr);
      exit (1);
    }
  n_banks = ((int) ucon64.fsize / 0x8000) * 2;  // how many 16k banks (rounded
  ...
  for (bank = 0; bank < n_banks; bank++)
    {
      ...
      n_bytes += 0x4000;
      ucon64_gauge (starttime, n_bytes, (size_t) ucon64.fsize);
    }
  ...
  for (bank = 0; bank < n_banks; bank++)
    {
      ...
      n_bytes += 0x4000;
      ucon64_gauge (starttime, n_bytes, ucon64.fsize);
    }
#endif
...
gbx_write_sram (const char *filename, unsigned short parport, int start_bank)
{
  ...
  if (start_bank == -1)
    {
      start_bank = 0;
      n_banks = MIN (n_banks, ((unsigned int) ucon64.fsize + 8192 - 1) / 8192); // "+ 8192 - 1" to round up
    }
  else
    {

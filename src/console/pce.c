...
*/
{
  ...
  strcpy (dest_name, ucon64.fname);
  ucon64_file_handler (dest_name, src_name, 0);
  fcopy (src_name, 0, ucon64.fsize, dest_name, "wb"); // no copy if one file

  if ((bytesread =
         ucon64_fread (buffer, rominfo->backup_header_len, 32 * 1024, src_name)) == 0)
    return -1;
...
static void
write_game_table_entry (FILE *destfile, int file_no, size_t totalsize,
                        unsigned int size)
{
  ...
  n = (int) strnlen (p, 0x1c);
  if (n < 0x1c)                                 // warning remover
  ...
  fputc ((int) (totalsize / MBIT), destfile);   // 0x1d = bank code
...
pce_init (st_ucon64_nfo_t *rominfo)
{
  ...
  unsigned int size, x, pos = (unsigned int) strlen (rominfo->misc);
  unsigned char *rom_buffer;

  x = ucon64.fsize % (16 * 1024);
  rominfo->backup_header_len = UCON64_ISSET2 (ucon64.backup_header_len, unsigned int) ?
                                 ucon64.backup_header_len : ucon64.fsize > x ? x : 0;
